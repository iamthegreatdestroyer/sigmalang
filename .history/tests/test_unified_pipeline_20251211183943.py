...
class TestUnifiedAnalogyPipeline:
    ...
    def test_process_query_caching(self, pipeline):
        """Test that results are cached for subsequent lookups."""
        # Use a query that will use STANDARD strategy (not CACHE_ONLY)
        # by providing longer content that triggers SIMPLE+ complexity
        query = Query(
            id="cache_test_query",
            content="A is to B as C is to D and E is to F - this is a longer query"
        )
