...
class TestEdgeCases:
    ...
    def test_weight_learner_extreme_outcomes(self):
        ...
        # Extreme success (True outcome with high score = high performance)
        ...
        # Extreme failure (False outcome with low score = high performance that converts to low)
        learner.update_weight("p_extreme_fail", False, 0.99999)
        weight_fail = learner.get_pattern_weight("p_extreme_fail")
        # False outcome with high score converts to (1 - 0.99999) = 0.00001, so weight should be very low
