...
class TestCallbackSink:
    ...
    def test_callback_called(self):
        ...
        received = []
        def callback(x):
            return (received.append(x), True)[1]
...
class TestBackpressureHandler:
    ...
    def test_backpressure_activation(self):
        ...
        # Fill to high watermark (8/10 = 80%)
        for i in range(8):
            handler.push(i)

        # Need to push one more to trigger the watermark check
        handler.push(8)

        assert handler.is_backpressure_active is True

        # Drain below low watermark (need less than 50% = 5 items)
        for _ in range(6):  # Remove 6 items, leaving 3
            handler.pop(block=False)

        # Push one to re-evaluate watermarks at 4/10 = 40% (below low_watermark)
        handler.push(100)
