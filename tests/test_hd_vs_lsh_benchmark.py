...
# Import from local benchmarking utils
try:
    ...
    from tests.benchmarking_utils import (
        BenchmarkResult,
        BenchmarkSuite,
        ComparativeResult,
        DatasetComplexity,
    ...
    def test_similarity_computation_latency(
        ...
        def lsh_similarity():
            ...
            for i in range(9):
                ...
                hash(tuple((v1 > 0).astype(int)))
                hash(tuple((v2 > 0).astype(int)))
                t1 = time.perf_counter()
